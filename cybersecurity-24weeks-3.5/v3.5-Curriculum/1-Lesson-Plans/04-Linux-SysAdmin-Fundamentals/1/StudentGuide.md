...
- [x] [01. So, Why Linux?](StudentGuide.md#01-so-why-linux)
- [x] [02. Linux History and Distributions](StudentGuide.md#02-linux-history-and-distributions)
- [x] [03. Activity: Distribution Research](StudentGuide.md#03-activity-distribution-research)
- [x] [04. Review: Distribution Research](StudentGuide.md#04-activity-review-distribution-research)
- [x] [05. Linux File System Structure](StudentGuide.md#05-linux-file-system-structure)
- [x] [06. Activity: Linux Landmarks](StudentGuide.md#06-activity-linux-landmarks)
- [x] [07. Review: Linux Landmarks](StudentGuide.md#07-activity-review-linux-landmarks)
- [x] [08. Break](StudentGuide.md#08-break)
- [x] [09. Resources and Processes](StudentGuide.md#09-resources-and-processes)
- [x] [10. Activity: Process Investigation](StudentGuide.md#10-activity-process-investigation)
- [x] [11. Review: Process Investigation](StudentGuide.md#11-activity-review-process-investigation)
- [x] [12. Installing Packages](StudentGuide.md#12-installing-packages)
- [x] [13. Student Take-Home Activity: Installing Packages](StudentGuide.md#13-take-home-activity-installing-packages)
- [x] [14. Take-Home Activity Review: Installing Packages](StudentGuide.md#14-take-home-activity-review-installing-packages)
...
While Linux is uncommon as an operating system on personal computers, it is widespread on the internet and in enterprise networks. Millions of websites are hosted by Linux operating systems.
...
You may hear Unix and Linux used interchangeably, but what exactly is the difference?
...
- Being able to navigate a headless Linux server is the main reason you need to learn Linux and the bash command line. Learning the bash command line environment will enable you to work with both Linux and Unix systems, such as Mac OS X.
...
- You therefore can apply what you learn today to other distributions you encounter, as they all have the files and folders you will learn about today.

- In addition, all popular Linux distributions use the bash shell by default. This means that you can also use the command line tools you learn here on other distros.
...
- Before you look at the Linux file structure, it is important to know that the term `root` can have refer to multiple contexts.
   - Root can be either:
...
- You will further explore these in this section.
...
- We will start at the top of the directory structure with the Root Directory.
...
- If a row in `/etc/shadow` contains a `!` instead of a password hash, this means that the account is locked.

- If a row in `/etc/shadow` contains a `*` instead of a password hash, this means that the user is not allowed to login.

  - This is usually implemented on a user reserved for the system, and not a _human_ user.

#### /sbin and /bin Directories
Now we’ll look into several directories that contain program files run on the Linux distribution.
...
- Logs are meant to be permanent. Programs expect to add information to log files, and not to rewrite or delete them and create new ones (though administrators can do these things manually).
...
The CPU also works to process programs on a system:
...
- An extreme example of this would be an attacker intentionally using up a system's resources to slow the machine down and compromise its availability. This type of DoS attack is often carried out on a web server, and stops the machine from being able to display its website. 
...
The bottom pane breaks down usage by each individual process running on the system.
...
- **NI:** "Nice value" is related to the process priority. A nice value of -20 means "highest process priority." A nice value of 19 means "lowest process priority." Most processes have a nice value of zero. A process priority can be escalated in a number of ways, though it is normally managed automatically.
...
- This output lists all processes sorted by the user that started them. It gives you a snapshot of all of the processes running on the machine at the time you ran the command.
...
This output is much more manageable than sorting through every process manually.
...
In order to both process and inspect the results with `top`, you'll need two terminals: one to start the script, and another to run `top`.
...
Note one of the `stress` PIDs you want to kill.  Remember that the PID is the first column from the left.
...
While this example is simple for ease of demonstration, the `kill` processes are important for controlling exactly what is running on a host, which is important for both sysadmins and security professionals.
...
- These databases are known as repositories. Repositories are specifically used to store and distribute packages known as Personal Package Archives, or PPAs.

- PPAs are simply servers where Linux software is stored and maintained.

Now that we have stopped a malicious service from running and completed a basic audit, we're going to take a look at some of the miscellaneous utilities that we can install onto our VMs. Specifically we'll be installing the following services:
