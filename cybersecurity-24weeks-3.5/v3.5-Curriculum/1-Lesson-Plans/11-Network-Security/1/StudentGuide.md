...
In the GRC module, we covered how defense-in-depth plays a critical role in securing organizations. Throughout this module, we will apply a DiD mindset to layer security measures that aim to slow an attack's progression, thus providing network defenders with enough time to respond:

- Today, we will cover the benefits and limitations for various types of firewalls. We will also explore defense-in-depth methodologies.
...
- **IP (Internet Protocol):** IP is responsible for addressing devices on a network and ensuring the delivery of data from a source to a destination.
...
This allows stateful firewalls to detect more information than stateless firewalls can. For example, they can determine whether:

  - A packet is trying to establish a new connection. This is called a **NEW** state.
 
  - A packet is part of an existing connection. This is called an **ESTABLISHED** state.
 
  - A packet is not opening a new connection nor does it belong to an existing one. This is considered a **ROGUE** packet.
...
- For example: Stateful firewalls can identify that a connection is using HTTP, but it cannot identify if the connection is being used to request an HTML file or a PNG image.
...
- One disadvantage is that they do not check the contents of the packet itself.
  ...
  - This is why circuit-level gateways are not capable of fully protecting a network on their own.
...
6. Denying traffic by default breaks networked applications, since it prevents them from sending or receiving data. Next, we'll need to explicitly allow traffic to and from ports `80`, `443`, and `22`. 
   ...
   ```
    ...
    - `80, 443, 22, 110` are the ports to be opened.
   ...
   - If we want to verify that these rules take effect, we need to run `sudo ufw status`.
...
7. You can delete firewall rules when they are no longer needed.
   ...
   - `deny 110` is the rule to be removed.
...
Different interfaces may require different firewall rules due to the various services that may be in use.
...
1. Since firewalld is dynamic, we can start it and make changes without having to restart or reload it. We start firewalld using the following command:
   ...
   - `sudo /etc/init.d/firewalld start`
        ...
        - `/etc/init.d/firewalld`is the file path to the firewalld program.
   ...
   - The firewalld service is located in the `etc/init.d/firewalld` directory.
...
2. With firewalld, we can set a zone to an interface and configure each individual zone setting. We can use `firewall-cmd` to list zones and services, associate zones with interfaces, and configure new firewall rules.
   ...
   - `sudo firewall-cmd --list-all-zones`
        ...
        - `firewall-cmd` is the option that tells firewalld to create, modify, and delete rules.
        - `--list-all-zones` is the command option that lists all currently configured zones.
...
3. Since we know that the zone will be work-related and the physical interface located on the third floor is `eth1`,  we need to bind the `work` zone to the `eth1` interface.
   ...
   - `sudo firewall-cmd --zone=work --change-interface=eth1`
        ...
        - `eth1` is the interface used to change to.
...
4. Let's verify the binding of the `work` zone to the physical interface of `eth1`.
   ...
   Output should look similar to:
     ...
     It's important to know which serivces are running on your system. 
...
5. Next, we will block all traffic coming from the Fifth Street location. The IP address associated with that location is `10.10.0.10`.
   ...
   - `sudo firewall-cmd --zone=work --add-rich-rule='rule family="ipv4" source address="10.10.0.10" reject'`
        ...
        - `--add-rich-rule=` is the option to add a new rich rule.
        - `rule family="ipv4"` limits the rule to the IPV4 protocol.
        - `source address="10.10.0.10"` is the source IP address.
        - `reject` is the option to reject IPV4 addresses from the source address.
...
2. Now we'll simulate an OS scan, which will enumerate OS names and versions. With this information, attackers know which specific vulnerabilities exist then attack those.
   ...
   - `sudo nmap -O -p 1-500 --osscan-guess 172.17.18.72`
      ...
      - `nmap` is the program to perform the network scan.
