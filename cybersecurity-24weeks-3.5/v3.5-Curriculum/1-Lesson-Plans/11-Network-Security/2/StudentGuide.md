...
**Intrusion detection systems** (**IDS**) are tools that can both analyze traffic and look for malicious signatures. An IDS is similar to a firewall but has additional capabilities. For example, it reads the data in the packets it inspects, issues alerts/alarms, and blocks malicious traffic if configured to do so.
...
**Cyber Threat Hunting** is a cyber defense methodology and important skill of a cybersecurity analyst used to provide network security through network defense processes.
  ...
  - It's a process that proactively and interactively searches through networks to identify and contain threats designed to evade existing security controls.

  - It provides the security analyst with a high-level overview of the tactics, techniques, and procedures or **TTPs** used by adversaries.

  - An iterative process which must be continuously performed in a loop that all starts with a hypothesis, typically an IOA or IOC.

Explain that Cyber Threat Hunting follows three basic methodologies:

1. **Analytics Driven:** This is the process of using indicators to derive a hypothesis upon which to investigate.

2. **Intelligence Driven:** This uses threat intelligence reports, network forensics, and malware analysis to establish adversarial TTPs.

3. **Situational Awareness Driven:** This uses risk assessments to remediate and mitigate threats.
...
- IOA indicates attacks happening in real time. It has the following characteristics:
   - Proactive approach to intrusion attempts
   - Indicates that an attack is currently in progress but a full breach has not been determined or has not occurred yet 
   - Focuses on revealing the intent and end goal of the attacker regardless of the exploit or malware used in the attack

- **Indicators of compromise** (**IOC**) indicate previous malicious activity. They have the following characteristics:
...
- **NetworkMiner:** Performs advanced network traffic analysis. Extracts artifacts from PCAP files and provides an intuitive user interface with which to analyze them. Allows the analyst to analyze, reassemble, and regenerate transmitted files and certificates from PCAP files.
...
Sguil has six key functions that help NSM analysts with their work:
  ...
  - It allows easy transitions between alert or session data and full content data, which is rendered as text in a transcript or in a protocol analyzer like Wireshark.

  - It exposes features so analysts can count and classify events, enabling escalation and other incident response decisions.
...
2. **Snort Rule:** The Snort rule that generated the alert, obtained from the IDS engine
...
- The source port and protocol used are in Box 3.

- The protocols used during transmission are in Box 4.
